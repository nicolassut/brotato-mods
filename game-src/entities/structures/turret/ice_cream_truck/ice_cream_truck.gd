...
# Gourmet DLC - Ice Cream Truck: serves an Ice Cream every 6 seconds (structure
# attack speed applies) while a live player is inside its serving radius.
# Modeled on the Garden (spawning turret); food spawning routes through the
# wanted_to_spawn_food structure signal handled in main.gd.


export (String) var anchored_food = "consumable_food_ice_cream"
...
func _is_player_in_radius() -> bool:
	# same current_scene access pattern the Turret base already uses in _ready
	for player in get_tree().current_scene._players:
		if not player.dead and global_position.distance_to(player.global_position) <= stats.max_range:
			return true
	return false
