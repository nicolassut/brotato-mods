...
MOD_NS = "nicolassut-NewItems"
ICON_SRC_DIR = "/Users/nicolassutcliffe/brotato-mods/Brotato Icons/scaled_96"

# id -> (display name, tier, value, [(stat_key, value), ...], notes)
ITEMS = [
    ("vampire_fang", "Vampire Fang", 4, 110,
        [("stat_lifesteal", 15), ("stat_max_hp", -5)],
        "simplified: original shield-overheal mechanic dropped, needs custom scripting"),
    ...
    ("overtime_pay", "Overtime Pay", 3, 80,
        ...
        "simplified: original per-wave-scaling-with-cap mechanic dropped, flat approximation"),
    ("second_mortgage", "Second Mortgage", 3, 70,
        [("gain_pct_gold_start_wave", -15), ("percent_materials", 10)],
        "simplified: -15% start-of-wave is exact; +10% ongoing materials substitutes for +30% at wave end"),
    ("voodoo_potato", "Voodoo Potato", 3, 70,
        [("stat_elemental_damage", 10), ("stat_max_hp", -5)],
        "simplified: curse DLC mechanic unverifiable locally (not present in this install), substituted elemental damage"),
...
item_ext_paths = []  # (my_id, res_path_to_item_tres)

for item_id, name, tier, value, effects, notes in ITEMS:
    item_dir = f"content/items/{item_id}"
    res_dir = f"res://mods-unpacked/{MOD_NS}/{item_dir}"

    # copy icon png
    src_png = os.path.join(ICON_SRC_DIR, f"{item_id}.png")
    dst_png = os.path.join(MOD_ROOT, item_dir, f"{item_id}.png")
    ...
    write(os.path.join(MOD_ROOT, item_dir, f"{item_id}.png.import"),
          png_import_tres(png_res_path, stex_hash))

    # effect .tres files
    effect_ext_lines = []
    effect_ext_ids = []
    load_id = 3  # id 1 = item_data.gd script, id 2 = icon texture, effects start at 3
    for i, (key, val) in enumerate(effects, start=1):
        fname = f"{item_id}_effect_{i}.tres"
        write(os.path.join(MOD_ROOT, item_dir, fname), effect_tres(key, val))
        eid = load_id
        load_id += 1
        effect_ext_ids.append(eid)
        effect_ext_lines.append(
            f'[ext_resource path="{res_dir}/{fname}" type="Resource" id={eid}]'
        )
...
tags = [ {tags_str} ]
'''
    item_res_path = f"{res_dir}/{item_id}.tres"
    write(os.path.join(MOD_ROOT, item_dir, f"{item_id}.tres"), item_tres)
    item_ext_paths.append((item_id, name, item_res_path, notes))

print("Items built:")
for item_id, name, path, notes in item_ext_paths:
    print(f"  {item_id}: {path}  ({notes})")

with open("/tmp/item_ext_paths.txt", "w") as f:
    for item_id, name, path, notes in item_ext_paths:
