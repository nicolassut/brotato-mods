...
`~/brotato-decompiled/` is **not** a git repo, so its non-mirrored contents have no history at
all — back it up too when touching it.
...
- `~/brotato-decompiled/` — the live game. Not version controlled.
